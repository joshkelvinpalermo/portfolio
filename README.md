...
* How it works: The raw data are being extracted from SAPS4/HANA using an application built on Azure App Service, then dumped on Data Lake. Using the JSON configuration files that we developed, these data are processed and transformed based on the rules, mappings, and transformations are defined, including the join, primary, and surrogate keys. The purpose of these JSON files were so we can just dynamically apply several transformation methods using only one framework. Cleaned and structured data would then be stored on the Delta Lake and is loaded to Tableau as the serving layer.
